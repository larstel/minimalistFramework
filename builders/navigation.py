...
def build_navigation(build_config, page_file_name, language_code, general_localization, page_content, translations):
    if(page_file_name not in build_config["noNavigation"]):
        ...
        for content in build_config["content"]:
            file_list = os.listdir("../" + build_config["contentTemplatesPath"] + "/" + content + "/")
            ...
            for page in sortedList:
                ...
                translation_dict_of_list = json.load(open("../" + build_config["contentTemplatesPath"] + "/" + content + "/" + page.split(".")[0] + "_localization.json"))
